// 7_1 다중 접속 1대 다 통신 서버.cpp
/*
1. 통신 소켓 관리
-> vector <SOCKET> sockets
2. 연결된 위치에서 저장					( accept 이후) 
3. 연결이 종료 되었을 때 삭제.			( 통신 스레드 종료 시점 ) 
4. 수신된 데이터를 모두에게 전송시 사용	( 통신 스레드 send 다음 ) 
*/
...
unsigned long CALLBACK work_thread(void* p)
{
	...
	while (true)
	{
		...
		else
		{
			...
			{
				SOCKET s = sockets[i];
				int send_byte = send(s, buf, (int)strlen(buf) + 1, 0);	
				printf(">>[송신] %dbyte\n\n", send_byte);
			}		
		}
	}
